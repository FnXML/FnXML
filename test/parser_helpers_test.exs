...
defmodule FnXML.Parser.AttributesTest do
  ...
  describe "attributes" do
    ...
    test "single attribute" do
      assert AttributesTest.attributes("name='value'") |> elem(1) == [
    ...
    test "multiple attributes" do
      ...
      assert AttributesTest.attributes(attrs) |> elem(1) == [
...
defmodule FnXML.Parser.ElementTest do
  ...
  describe "tags" do
    ...
    test "element with attributes" do
      assert ElementTest.open_tag("tag a='1' b=\"2\" c='3'") |> elem(1) == [
               open: [
                 tag: "tag",
                 attributes: [{"a", "1"}, {"b", "2"}, {"c", "3"}],
                 loc: {1, 0, 0}
  ...
  describe "comment" do
    test "simple comment" do
      assert ElementTest.comment_tag("!-- comment --") |> elem(1) == [
               comment: [content: " comment ", loc: {1, 0, 0}]
             ]
    end
  ...
  describe "processing_instruction" do
    test "basic" do
      assert ElementTest.pi("?id string of text?") |> elem(1) == [
               proc_inst: [tag: "id", content: "string of text", loc: {1, 0, 0}]
             ]
    end
  end
  ...
  describe "prolog" do
    test "basic" do
      assert ElementTest.prolog("<?xml version='1.0' encoding='UTF-8'?>") |> elem(1) == [
               prolog: [
                 tag: "xml",
  ...
  describe "text" do
    test "simple" do
      assert ElementTest.text("hello world") |> elem(1) == [
               text: [content: "hello world", loc: {1, 0, 0}]
             ]
    end
    ...
    test "terminated with '<'" do
      assert ElementTest.text("hello<other stuff") |> elem(1) == [
               text: [content: "hello", loc: {1, 0, 0}]
             ]
    end
  end
  ...
  describe "cdata" do
    test "simple" do
      assert ElementTest.cdata("![CDATA[hello world]]") |> elem(1) == [
               text: [content: "hello world", loc: {1, 0, 0}]
             ]
    end
  end
