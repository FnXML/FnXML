defmodule FnXML.Stream.NativeDataStruct.Encoder.DefaultTest do
  ...
  describe "text keys" do
    ...
    test "default 2" do
      assert NDS.Encoder.Default.text_keys(%NDS{}, %{"t" => "b", "#" => "a", "text" => "c"}) == [
               "text",
               "t",
               "#"
             ]
    ...
    test "default with non-text excluded" do
      assert NDS.Encoder.Default.text_keys(%NDS{}, %{"a" => "b", "#" => "a", "text" => "c"}) == [
               "text",
               "#"
             ]
    end
    ...
    test "opts text keys, all keys found" do
      ...
      assert NDS.Encoder.Default.text_keys(nds, %{"a" => "b", "b" => "a", "text" => "c"}) == [
               "a",
               "b"
             ]
    end
  ...
  describe "key order" do
    test "using opts[:order]" do
      ...
      assert NDS.Encoder.Default.key_order(nds, %{"a" => "t1", "b" => "t2"}, ["a", "b"], [
               "a",
               "b"
             ]) == [{:text, "a", "t1"}, {:text, "b", "t2"}]
    end
    ...
    test "with simple text and child elements" do
      ...
      assert NDS.Encoder.Default.key_order(nds, data, [], []) == [
               {:child, "a", "hello"},
               {:child, "b", "world"}
             ]

      assert NDS.Encoder.Default.key_order(nds, data, [:a], ["b"]) == [
               {:text, "b", "world"},
               {:child, "a", "hello"}
             ]
    end
    ...
    test "with child element" do
      ...
      assert NDS.Encoder.Default.key_order(%NDS{}, data, [:c], ["a"]) == [
               {:text, "a", "hello"},
               {:child, "b", %{"info" => "info", a: 1, b: 1}}
             ]
    end
    ...
    test "with text list element and child element" do
      ...
      assert NDS.Encoder.Default.key_order(%NDS{}, data, [:c, :d], ["a"]) == [
               {:text, "a", "hello"},
               {:text, "a", "again"},
               {:child, "b", %{"info" => "info", a: 1, b: 1}}
             ]
  ...
  describe "child generator:" do
    ...
    test "basic children" do
      ...
      assert NDS.Encoder.Default.content(%NDS{}, data) |> NDS.TestHelpers.clear_private() == %NDS{
               content: [
                 {:text, "text", "info"},
                 {:child, "c1",
                  %NDS{tag: "c1", attributes: [{"a", "1"}], content: [{:text, "t", "child"}]}},
                 {:child, "c2",
                  %NDS{tag: "c2", attributes: [{"a", "2"}], content: [{:text, "t", "child"}]}}
               ]
             }
    end
    ...
    test "child list" do
      ...
      assert NDS.Encoder.Default.content(%NDS{}, data) |> NDS.TestHelpers.clear_private() == %NDS{
               content: [
                 {:child, "c",
                  %NDS{tag: "c", attributes: [{"a", "1"}], content: [{:text, "t", "first"}]}},
                 {:child, "c",
                  %NDS{tag: "c", attributes: [{"a", "2"}], content: [{:text, "t", "second"}]}}
               ]
             }
    end
                    ...
                    content: [
                      ...
                      {:child, "c1.1",
                       %NDS{
                         tag: "c1.1",
                         attributes: [{"b", "1"}],
                         content: [{:text, "t", "child.1"}]
                       }},
                      {:child, "c1.2",
                       %NDS{
                         tag: "c1.2",
                         attributes: [{"b", "2"}],
                         content: [{:text, "t", "child.2"}]
                       }}
                    ]
                  }},
                 {:child, "c2",
