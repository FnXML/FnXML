defmodule FnXML.DTD.EntityResolver do
  @moduledoc """
  DTD-aware entity resolution using composition with `FnXML.Entities`.

  This module extracts entity definitions from a `FnXML.DTD.Model` and delegates
  the actual resolution to `FnXML.Entities`, adding DTD-specific features
  ...
  - `:on_unknown` - How to handle unknown entities (passed to FnXML.Entities)
  ...
  delegates to `FnXML.Entities.resolve/2`.
  ...
  passing to `FnXML.Entities.resolve/2`.
