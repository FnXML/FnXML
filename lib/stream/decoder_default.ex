defmodule FnXML.Stream.Decoder.Default do
  ...
  """
  def handle_open({:start_element, tag, attrs, loc}, _path, acc, _opts) do
  ...
  """
  def handle_text({:characters, content, _loc}, _path, [h | t], _opts) do
    [[{:text, content} | h] | t]
  end
  ...
  def process_element(element, path, opts) do
    handler =
      Keyword.get(opts, :handle_element, fn element, _path, _opts -> {:child, element} end)
  ...
  @impl true
  def handle_proc_inst({:processing_instruction, name, content, loc}, _path, acc, _opts) do
