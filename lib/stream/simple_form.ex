defmodule FnXML.Stream.SimpleForm do
  @moduledoc """
  ...
  - Struct-based representation with `%FnXML.DOM.Element{}`
  - Document-level metadata with `%FnXML.DOM.Document{}`
  ...
  ## Examples
      ...
      iex> FnXML.Stream.SimpleForm.decode("<root><item id=\"1\">Hello</item></root>")
      {"root", [], [{"item", [{"id", "1"}], ["Hello"]}]}

      # Encode SimpleForm back to XML
      iex> FnXML.Stream.SimpleForm.encode({"root", [], [{"item", [{"id", "1"}], ["Hello"]}]})
      "<root><item id=\"1\">Hello</item></root>"

      # Convert FnXML stream to SimpleForm
      iex> FnXML.Parser.parse("<a>text</a>")
      ...> |> FnXML.Stream.SimpleForm.from_stream()
      {"a", [], ["text"]}

      # Convert SimpleForm to FnXML stream
      iex> {"a", [], ["text"]}
      ...> |> FnXML.Stream.SimpleForm.to_stream()
      ...> |> Enum.to_list()
  ...
  ## Examples
      ...
      iex> FnXML.Stream.SimpleForm.decode("<root><child>text</child></root>")
      {"root", [], [{"child", [], ["text"]}]}

      iex> FnXML.Stream.SimpleForm.decode("<root attr=\"val\"/>")
      {"root", [{"attr", "val"}], []}
  ...
  ## Examples
      ...
      iex> FnXML.Stream.SimpleForm.encode({"root", [], ["text"]})
      "<root>text</root>"

      iex> FnXML.Stream.SimpleForm.encode({"div", [{"class", "container"}], []})
  ...
  ## Examples
      ...
      ...> |> FnXML.Stream.SimpleForm.from_stream(include_comments: true)
  ...
  # Handle text - add to current element's content
  defp handle_event(
  ...
  # Handle processing instructions (ignore for SimpleForm compatibility)
  defp handle_event(
  ...
  # Handle errors (ignore for SimpleForm compatibility)
  ...
  ## Examples
      ...
      iex> {"root", [{"id", "1"}], ["text"]}
      ...> |> FnXML.Stream.SimpleForm.to_dom()
      %FnXML.DOM.Element{tag: "root", attributes: [{"id", "1"}], children: ["text"]}
  ...
  ## Examples
      ...
      iex> %FnXML.DOM.Element{tag: "root", attributes: [{"id", "1"}], children: ["text"]}
      ...> |> FnXML.Stream.SimpleForm.from_dom()
      {"root", [{"id", "1"}], ["text"]}
